...
const ConfirmLogin = observer(({
  ...
  const submit = () => {
    ...
    setTimeout(() => {
      const destination = store.authentication.mfa === 'email' ? store.authentication.email.toLowerCase() : store.authentication.phone;
      store.authentication.confirm(code, destination)
        .then((result) => {
          if (result) {
            store.authentication.get(store.authentication.accessToken)
              .then(() => {
        ...
        .catch((e) => {
          console.warn(e);
          if (e === 'invalid') {
            setError(translator.t('global.mfa.errors.invalidCode'));
          }
          else {
            setError(translator.t('global.mfa.errors.unknown'));
          }
          store.display.hideSpinner();
        });
    }, 2000);
  ...
  return (
    ...
    >
      ...
      <ScrollView
        ...
        <Text style={styles.title}>{translator.t('global.mfa.confirm.title')}</Text>
        ...
        <View style={styles.resendContainer}>
          <Text style={styles.resendText}>{translator.t('global.mfa.confirm.resendText')}</Text>
